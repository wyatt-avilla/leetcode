...
impl Solution {
    pub fn camel_match_old(queries: Vec<String>, pattern: String) -> Vec<bool> { // two pointers instead?
        let pattern_capital_indices: Vec<_> = pattern.chars().enumerate()
    ...
    pub fn camel_match(queries: Vec<String>, pattern: String) -> Vec<bool> {
        ...
        'outer: for word in queries {
            ...
            for (&widx, &pidx) in word_capital_indices.iter().zip(pattern_capital_indices.iter()) {
                if widx == 0|| pidx == 0 {continue};
                let subword = &word[l_word_bound..widx];
                let subpat = &pattern[l_pat_bound..pidx];
                println!("{}[{}..{}] = {} and {}[{}..{}] = {}", &word, l_word_bound, widx, subword, pattern, l_pat_bound, pidx, subpat);
                println!("{} can be expanded to {}, {}", subpat, subword, Self::expandable_to(subpat, subword));
                if !Self::expandable_to(subpat, subword) { matches.push(false); continue 'outer }

                l_word_bound = widx;
                l_pat_bound = pidx;
            }
            println!("final {} and {}", &pattern[l_pat_bound..], &word[l_word_bound..]);
            matches.push(Self::expandable_to(&pattern[l_pat_bound..], &word[l_word_bound..]));
        }
        matches
    }
}
...
mod tests {
    ...
    #[test]
    fn case1() {
        let queries = vec![String::from("FooBar"), String::from("FooBarTest"), String::from("FootBall"), String::from("FrameBuffer"), String::from("ForceFeedBack")];
    ...
    #[test]
    fn case4() {
        let queries = vec![String::from("CompetitiveProgramming"), String::from("CounterPick"), String::from("ControlPanel")];
